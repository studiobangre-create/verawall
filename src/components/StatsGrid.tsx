...
export function StatsGrid({ title, stats }: { title: string; stats: Stat[] }) {
  ...
  return (
    <section style={{ maxWidth: 'var(--page-width)', margin: '0 auto', padding: '0 15px 90px' }}>
      <h2 style={{ fontSize: isMobile ? 28 : 40, fontWeight: 700, textAlign: 'center', color: '#5A6976', textWrap: 'balance' }}>{t(title)}</h2>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: 28, maxWidth: 'var(--page-width)', margin: '52px auto 0' }}>
        {stats.map((st) => (
